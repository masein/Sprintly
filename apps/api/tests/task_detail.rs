...
async fn make_project_with_task(pool: &PgPool, owner: Uuid) -> (Uuid, Uuid) {
    ...
    ).bind(pid).bind("TST").bind("T").bind(owner).execute(pool).await.unwrap();
...
async fn attachment_lifecycle_pending_then_ready(pool: PgPool) {
    ...
    .bind(aid).bind(tid).bind(owner).execute(&pool).await.unwrap();
    ...
    let (status, size): (String, Option<i64>) = sqlx::query_as(
        "SELECT status, size_bytes FROM task_attachments WHERE id = $1",
    ).bind(aid).fetch_one(&pool).await.unwrap();
